...
"""
struct VoronoiBinner{Dim, M, CRS} <: BinningAlgorithm{Dim}
...
function VoronoiBinner(
    ...
    hardclip::Bool = true) where {Dim, M, CRS}
...
membership(data::Matrix{<:Real}, binner::VoronoiBinner{1, M, CRS}) where {M, CRS} = _membership1d(data, binner.bins)
membership(traj::Trajectories{1}, binner::VoronoiBinner{1, M, CRS}) where {M, CRS} = _membership1d(traj, binner.bins)

membership(data::Matrix{<:Real}, binner::VoronoiBinner{2, M, CRS}) where {M, CRS} = _membership2d(data, binner.bins)
membership(traj::Trajectories{2}, binner::VoronoiBinner{2, M, CRS}) where {M, CRS} = _membership2d(traj, binner.bins)