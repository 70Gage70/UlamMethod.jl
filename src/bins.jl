...
struct HyperRectangleBinner{Dim, CRS, R<:AbstractRange} <: BinningAlgorithm{Dim}
    nbins::Int64
    boundary::Boundary{Dim, Dim, CRS}
    ranges::Vector{R}
    
    function HyperRectangleBinner(nbins::Int64, boundary::Boundary{K, Dim, CRS}) where {K, Dim, CRS}
...
function _bin(boundary::Boundary{K, Dim, CRS}, binner::HyperRectangleBinner{Dim}) where {K, Dim, CRS}
    verts = Iterators.product(binner.ranges...) |> collect
    idx = Iterators.product(axes(verts)...) |> collect

    bins = Polytope{K, Dim, CRS}[]
