...
n_polys_removed = n_polys_requested - length(keys(ip0.contains))
n_polys_dis = length(polys_dis)
n_counts_total = sum(counts) 
n_counts_removed_scc = n_counts_total - sum(counts[largest_component])
info = UlamInfo(n_polys_requested, n_polys_removed, n_polys_dis, n_counts_total, n_counts_removed_scc)
